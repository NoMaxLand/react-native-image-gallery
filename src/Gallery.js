    ...
    let onResponderReleaseOrTerminate = (evt, gestureState) => {
      if (this.activeResponder) {
        if (
          ...
          if (
            (Math.abs(gestureState.vy) > 1.5 ||
              Math.abs(gestureState.dy) >
                Dimensions.get("window").height / 2) &&
            this.getImageTransformer(this.currentPage).state.scale === 1
          ) {
            this.props.onSwipedVertical &&
              this.props.onSwipedVertical(evt, gestureState);
          }
